...
if not logs_reales:
    ...
    with col1:
        st.metric(label="💰 Monto Procesado (Hoy)", value=f"Bs. {monto_total:,.2f}")
    with col2:
        st.metric(label="🛑 Monto Retenido / Alertas", value=f"Bs. {monto_retenido:,.2f}", delta_color="inverse")
    with col3:
    ...
    # ==========================================
    # SECCIÓN: GRÁFICO DE TENDENCIA POR HORA
    # ==========================================
    ...
    if not aprobados.empty:
        ...
        st.bar_chart(data=grafico_data, x="Hora", y="monto", color="#25D366")
    else:
