...
def obtener_configuraciones():
    try:
        # Incluimos tipo_contenido en la consulta
...
def guardar_configuracion(palabras, contenido, tipo="text"):
    try:
        supabase = get_supabase()
        # 1. Guardamos respuesta con su tipo
...
def actualizar_configuracion(respuesta_id, contenido, tipo):
    try:
...
def eliminar_regla(cliente_id, respuesta_id):
    try:
        ...
        # Limpieza: borrar respuesta si nadie más la usa
